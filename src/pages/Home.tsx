...
const Home = () => {
    ...
    // Project Items for CircularGallery mapped from data
    const projectItems = portfolioData.projects.map(project => ({
        image: project.image,
        text: project.title,
        id: project.id
    }));
        ...
        <div className="bg-[#1a1a1a] text-white min-h-screen relative">
            ...
            <main className="relative z-10">
                <Section className="py-12 md:py-20">
                    <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12 text-center md:text-left">
                        <motion.div
                            initial={{ opacity: 0, scale: 0.8 }}
                            animate={{ opacity: 1, scale: 1 }}
                            transition={{ duration: 0.8 }}
                            className="relative w-64 h-64 md:w-96 md:h-96 flex-shrink-0 rounded-full p-[6px] bg-gradient-to-r from-purple-500 to-pink-500 shadow-2xl overflow-hidden mx-auto md:mx-0"
                        >
                        ...
                        <div className="space-y-4 md:space-y-6 max-w-lg">
                            ...
                            <div className="pt-4 flex justify-center md:justify-start">
                                <GitHubStats username={portfolioData.profile.github} />
                            </div>
                        </div>
                    </div>
                </Section>
                ...
                <Section className="h-[800px] block">
                    <div className="text-center mb-12">
                        ...
                        <p className="text-gray-400">Drag to explore • Click to view details</p>
                    </div>
                ...
                <Section>
                    <div className="max-w-6xl mx-auto w-full">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {portfolioData.profile.skills.map((tech) => (
                                <div key={tech} className="glass-panel p-6 rounded-2xl flex items-center justify-center text-center font-semibold hover:scale-105 transition-transform cursor-pointer">
                                    {tech}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-12 border-l-2 border-white/10 pl-8 ml-4">
                            ...
                            {portfolioData.experience.map((item, i) => (
                                <div key={i} className="relative">
                                    ...
                                    <div className="glass-panel p-8 rounded-xl">
                                        ...
                                        <p className="text-gray-400 mb-4 font-semibold">{item.company} | {item.location}</p>
                            ...
                            {portfolioData.education.map((item, i) => (
                                <div key={`edu-${i}`} className="relative">
                                    <div className="absolute -left-[41px] top-1 w-5 h-5 rounded-full bg-blue-500 border-4 border-[#1a1a1a]" />
                                    <div className="glass-panel p-8 rounded-xl">
                                        <div className="flex flex-col md:flex-row justify-between mb-2">
                                            <h3 className="text-xl font-bold text-white">{item.degree}</h3>
                                            <span className="text-sm text-blue-400">{item.period}</span>
                                        </div>
                                        <p className="text-gray-400">{item.school} | {item.location}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
